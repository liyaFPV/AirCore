...
void MeteoParsing(){
    ...
    for (size_t i = 0; i < data_count; i++) {
        ...
        Weather[0].time_str[i%24]=time_str;
        Weather[0].temp[i%24]=temp;
        Weather[0].humidity[i%24]=humidity;
        Weather[0].wind_speed[i%24]=wind_speed;
        Weather[0].wind_deg[i%24]=wind_deg;
        Weather[0].w_code[i%24]=w_code;
        Weather[0].w_code_str[i%24]=weather_codeParsing(w_code);
...
void getWeatherForecast(int count){
    ...
    int maxCount = 24 - startHour;
    if(count > maxCount) count = maxCount;
    for(int i = startHour; i < startHour + count; i++){
        int hour = i % 24;
        Serial.print(Weather[0].time_str[hour]);
        Serial.print(" | Temp: ");
        Serial.print(Weather[0].temp[hour]);
        Serial.print("C, Humidity: ");
        Serial.print(Weather[0].humidity[hour]);
        Serial.print("%, Wind: ");
        Serial.print(Weather[0].wind_speed[hour]);
        Serial.print(" m/s, Dir: ");
        Serial.print(Weather[0].wind_deg[hour]);
        Serial.print("deg, ");
        Serial.println(Weather[0].w_code_str[hour]);
    }
}